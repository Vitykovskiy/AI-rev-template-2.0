...
| `infrastructure` | `devops` | establish CI/CD, target-environment access, and bootstrap delivery prerequisites |
...
- `canonical_inputs` must point to repository artifacts or prerequisite tasks, including relevant approved library registry entries when library usage is material;
...
- `infrastructure` is a separate task type inside `issue_driven`; it may exist once per delivery stream and must finish before dependent implementation tasks move to `Ready`;
...
## Critical Precedence And Routing Rules

- resolve repository mode first: `current_stage` selects only `setup` or `issue_driven`;
- inside `issue_driven`, route by issue `task_type`, `owner_contour`, dependencies, and `project_status`;
- `deploy` and `e2e` are task types inside `issue_driven`, not repository-wide stages;
- task-type-specific handoff states override generic backlog pickup rules when they apply;
- legacy `instructions/e2e-test/*` files are deprecated redirects only and must not contain alternate execution logic.
...
- relevant approved entries in `docs/external-libraries.md` and `docs/internal-libraries.md` exist for libraries touched by the slice, or the slice explicitly records that no registry update is needed;
...
- setup is complete;
- the target environment, delivery path, and bootstrap access contract are identifiable;
- required operational prerequisites are explicit enough to implement without guessing.

Done when:

- CI/CD and target-environment bootstrap required for downstream work are verified;
- the repository-owned bootstrap contract for required secrets and environment placeholders is current;
- deployment prerequisites and operational handoff notes are recorded with explicit evidence;
- dependent implementation tasks can start without guessing about infrastructure state.
...
- relevant approved entries in `docs/external-libraries.md` and `docs/internal-libraries.md` are present in canonical inputs when the task materially depends on libraries, or the task explicitly records `none`;
...
- missing approved external or internal library contract -> block the current task and create or reopen a linked `system_analysis` follow-up task;
...
4. `block_delivery`
5. child `implementation` issues by contour
6. `deploy` when rollout is required
7. `e2e` when integrated validation is required
8. initiative closure after all required blocks are done
