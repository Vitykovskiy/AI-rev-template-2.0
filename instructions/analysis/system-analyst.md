...
- approved external dependency usage contracts for third-party libraries touched by the slice
- approved internal library usage contracts for shared modules touched by the slice
...
Any downstream task that materially depends on a third-party or shared internal library additionally requires:
- the relevant entry in `docs/external-libraries.md` and/or `docs/internal-libraries.md` is listed in canonical inputs;
- or the task explicitly records that no library registry entry is required.

## Produce

- complete canonical artifacts in `docs/analysis/`
- updated approved usage contracts in `docs/external-libraries.md` and `docs/internal-libraries.md` when the slice introduces, upgrades, or constrains library usage
...
- Before moving any library-dependent downstream task to `Ready`, confirm the relevant approved usage contract exists in `docs/external-libraries.md` and/or `docs/internal-libraries.md`, or explicitly record that no registry entry is required.
