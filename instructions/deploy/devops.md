...
- deployment result tied to the released artifact, commit, or build identifier
- rollout notes with executed steps, target environment, and health-check outcome
- environment updates or an explicit statement that none were required
- blockers for any release condition that is not met

## Evidence Contract

- identify what was deployed, where it was deployed, and which runbook or rollout path was used;
- record rollout verification, health checks, smoke checks, and their pass/fail outcome;
- record rollback status, mitigation steps, or explicit confirmation that rollback was not needed;
- record the downstream handoff state for `e2e`, including remaining risks or blockers.
