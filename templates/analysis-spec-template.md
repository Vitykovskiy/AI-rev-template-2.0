# Analysis Specification Template

## Initiative

- Name: `<initiative name>`
- Source intake artifact: `<link or reference>`

## Required Sections

1. Problem context
2. User scenarios
3. Version scope and acceptance
4. System modules
5. Domain model and data formats
6. Integration contracts
7. UI specification
8. Cross-cutting concerns
9. Library contract impact (`docs/external-libraries.md`, `docs/internal-libraries.md`, or explicit `none`)
10. Contour decomposition

## Definition Of Ready For Delivery

- [ ] Frontend can work from UI specs and consumed contracts.
- [ ] Backend can work from module design, domain rules, and produced contracts.
- [ ] DevOps can work from runtime and operational requirements.
- [ ] QA E2E can work from user scenarios and acceptance criteria.
- [ ] Relevant external and internal library contracts are recorded or explicitly not needed.
- [ ] No contour needs to read sibling contour code to infer expected behavior.
