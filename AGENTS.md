...
After `setup`, work stays inside `current_stage = "issue_driven"` and follows this task chain:

1. `business_analysis`
2. `infrastructure`
3. one or more bounded `system_analysis` tasks
4. `block_delivery`
5. contour-owned `implementation` tasks
6. `deploy` when rollout is required
7. `e2e` when integrated validation is required

`deploy` and `e2e` are post-setup task types, not additional repository-wide `current_stage` values.
Infrastructure remains the initial delivery task and must be complete before any implementation task is created or starts.
